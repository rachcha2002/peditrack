...
const vaccine = () => {
  ...
  const fetchbabyProfilesFromFirestore = async () => {
    try {
      ...
      setLastCompletedVaccine(completedVaccines[completedVaccines.length - 1] || null);
      setNextPendingVaccine(pendingVaccines[0] || null);
      setCompletionPercentage(totalVaccines ? (completedVaccines.length / totalVaccines) * 100 : 0);
    } catch (error) {
  ...
  <Circle
    ...
    strokeDashoffset={(2 * Math.PI * (circleSize / 2 - 10) * (100 - completionPercentage)) / 100} // Adjust dash offset
    strokeLinecap="round"
